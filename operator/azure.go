...
const (
	optionAzureSubscriptionID = "azure-subscription-id"
	optionAzureResourceGroup  = "azure-resource-group"
)
...
func startAzureAllocator(clientQPSLimit float64, clientBurst int) error {
	...
	subscriptionID := viper.GetString(optionAzureSubscriptionID)
	if subscriptionID == "" {
		return fmt.Errorf("Azure subscription ID not specified")
	}

	resourceGroup := viper.GetString(optionAzureResourceGroup)
	if resourceGroup == "" {
	...
	nodeManager, err = ipam.NewNodeManager(instances, &k8sAPI{}, iMetrics, parallelAllocWorkers, false)
	if err != nil {
