...
func (e *IPAMSuite) TestIpamPreAllocate8(c *check.C) {
	...
	api := apimock.NewAPI([]*ipam.Subnet{testSubnet}, []*ipam.VirtualNetwork{testVnet})
...
func (e *IPAMSuite) TestIpamManyNodes(c *check.C) {
	...
	api := apimock.NewAPI(testSubnets, []*ipam.VirtualNetwork{testVnet})
	instances := NewInstancesManager(api)
	c.Assert(instances, check.Not(check.IsNil))
	...
	c.Assert(metrics.AvailableENIs(), check.Not(check.Equals), 0)
}
