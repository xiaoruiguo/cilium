...
func (e *MockSuite) TestMock(c *check.C) {
	api := NewAPI([]*ipam.Subnet{{ID: "s-1", AvailableAddresses: 100}}, []*ipam.VirtualNetwork{{ID: "v-1"}})
	...
	c.Assert(vnets["v-1"], checker.DeepEquals, &ipam.VirtualNetwork{ID: "v-1"})
	c.Assert(len(subnets), check.Equals, 1)
	c.Assert(subnets["s-1"], checker.DeepEquals, &ipam.Subnet{ID: "s-1", AvailableAddresses: 100})
...
func (e *MockSuite) TestSetMockError(c *check.C) {
	api := NewAPI([]*ipam.Subnet{}, []*ipam.VirtualNetwork{})
	c.Assert(api, check.Not(check.IsNil))
