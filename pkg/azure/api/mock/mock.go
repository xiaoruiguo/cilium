...
type API struct {
	...
	vnets     map[string]*ipam.VirtualNetwork
	errors    map[Operation]error
	delaySim  *helpers.DelaySimualtor
	limiter   *rate.Limiter
}

func NewAPI(subnets []*ipam.Subnet, vnets []*ipam.VirtualNetwork) *API {
	api := &API{
		instances: types.InstanceMap{},
		subnets:   map[string]*ipam.Subnet{},
		vnets:     map[string]*ipam.VirtualNetwork{},
...
func (a *API) UpdateSubnets(subnets []*ipam.Subnet) {
	a.mutex.Lock()
...
func (a *API) GetVpcsAndSubnets(ctx context.Context) (ipam.VirtualNetworkMap, ipam.SubnetMap, error) {
	a.rateLimit()
